...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            @if($produk->image)
            <img src="{{ asset('storage/' . $produk->image) }}" alt="{{ $produk->nama }}" class="w-64 h-64 object-cover mb-4 rounded-lg cursor-pointer" id="productImage">
            @endif

            <!-- Image Modal -->
            <div id="imageModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
                <div class="max-w-3xl max-h-full p-4">
                    <img src="{{ asset('storage/' . $produk->image) }}" alt="{{ $produk->nama }}" class="max-w-full max-h-full object-contain">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (productImage && imageModal && closeModal) {
