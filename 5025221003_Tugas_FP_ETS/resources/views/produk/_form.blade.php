...
<div class="mb-4">
    ...
    <input class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring focus:border-blue-300 @error('image') border-red-500 @enderror" id="image" type="file" name="image" accept="image/*">
    @error('image')
    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
    @enderror
</div>
...
<div class="mb-4">
    <img src="{{ asset('storage/' . $produk->image) }}" alt="{{ $produk->nama }}" class="rounded-lg w-auto h-64 object-cover">
</div>
@endif
